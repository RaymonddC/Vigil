...
> Final narration word count: **455 words** (~156 wpm)
...
- [ ] Next.js clinician dashboard on `localhost:3000`, all 4 views load without console errors
...
- [ ] Verify PT-001, PT-007, PT-009, PT-010 all appear in the patient list
...
| 0:15 | Cut to dashboard home, 4 patient cards | "Most of those deaths are missed signals — not missing data." | Zoom into patient grid | MATHUR |
| 0:20 | PT-001 card, green status | "This is Vigil. A clinician dashboard watching four post-op patients in a live FHIR server." | Highlight "FHIR" label | MANDEL |
| 0:25 | Click PT-001, vitals chart flat/green | "PT-001 is stable. Vitals flat, labs clean, our agent says: no action." | Green check animation | MATHUR |
| 0:30 | PT-001 detail view, "Agent: MONITOR" | "Notice what's missing — no alert. No false alarm." | Callout: "0 false alerts" | MATHUR |
| 0:35 | Hover FHIR badge showing resource IDs | "Every number on this screen is a real FHIR Observation pulled by an MCP tool." | Tooltip: `Observation/123` | MANDEL |
| 0:40 | Network panel showing `screen_vital_thresholds` call | "Same tool any MCP-compatible agent can call. SMART-on-FHIR under the hood." | Highlight tool name | MANDEL |
...
| 1:00 | Agent panel expands: "DETECTING" | "Vigil's agent sees the pattern across six hours of FHIR data and escalates." | Pulse animation | MATHUR |
| 1:05 | Four tool calls streaming in sidebar | "Four MCP tool calls — screen vital thresholds, score deterioration risk, flag sepsis onset, draft the SBAR." | Highlight each tool | MANDEL |
| 1:10 | `generate_escalation_note` row highlighted, draft SBAR appears | "The agent drafts the escalation, but never writes the chart on its own. Every action waits for a clinician." | Callout: "draft — not persisted" | PROCTOR |
...
| 1:40 | "Approve & Send RRT" button | "One click and the backend writes a FHIR Communication plus an AuditEvent to the chart." | Button click | PROCTOR |
| 1:45 | Confirmation toast: "Communication/comm-884 written" | "Closed loop — clinician approves, chart updates, audit trail captured. Agentic action, always with a human in the loop." | Toast fade | PROCTOR |
| 1:50 | Cut to PT-009 card, red urgent badge | "Now the part we're most proud of." | Hard cut | ZHENG |
...
> **Word count: 455 words. Target cadence: 156 wpm. Rehearse at 150 wpm to leave breathing room.**
...
| **Josh Mandel** (Microsoft, SMART on FHIR) | Real FHIR, real interop, reusability | 0:20, 0:35–0:40, 1:05, 2:00, 2:20–2:35 | "live FHIR server" call-out, tooltip with Observation IDs, tool-call stream, "zero code changes" diff, marketplace install | SERVED |
...
Show the boring case first. This is the Mathur moment: clinicians distrust AI because it cries wolf. Vigil's first demonstration is *silence* on a stable patient. Slip the Mandel hook in mid-scene — the FHIR tooltip — so interop is established before we get to the drama.
...
| 0:12–0:20 | SBAR typing in live | "It drafts the SBAR. The nurse sends it to Epic in one click." |
