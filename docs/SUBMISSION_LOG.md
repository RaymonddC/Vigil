...
| # | Item | Check | Signed off by | Notes |
|---|------|:-----:|:-------------:|-------|
| 13 | Demo video recorded, runtime **< 3:00** | [ ] | | Verify with stopwatch on playback, not trust. Hard ceiling per Devpost rules. |
| 14 | Video has captions (SRT uploaded or burned-in) | [ ] | | YouTube auto-caption → download SRT → hand-edit clinical terms → re-upload |
| 15 | Video resolution ≥ 1080p, MP4 H.264, < 500 MB | [ ] | | Devpost upload limit is 500 MB — aim for 300 MB |
| 16 | All 5 judge hooks present in video or README | [ ] | | Cross-reference `docs/JUDGE_HOOK_CHECK.md` — each judge has ≥3 hooks |

## Devpost Submission

| # | Item | Check | Signed off by | Notes |
|---|------|:-----:|:-------------:|-------|
| 17 | Devpost submission status: **Submitted** (not Draft) | [ ] | | Click "Submit" — drafts are NOT judged. Verify confirmation email. |
| 18 | Devpost fields complete | [ ] | | Title (≤70 chars), tagline (≤200 chars), description, built-with tags, video embedded, GitHub link, team roster. See `docs/DEVPOST_SUBMISSION.md` for copy. |
| 19 | Devpost category: **Option B** | [ ] | | Agents Assemble hackathon, correct track selected |
| 20 | Devpost team roster correct — every member has accepted the invite | [ ] | | Check "Team" tab — all members show "Accepted", not "Pending" |

## Marketplace

| # | Item | Check | Signed off by | Notes |
|---|------|:-----:|:-------------:|-------|
| 21 | Prompt Opinion MCP listing (Path A) live | [ ] | | Or KS-1 executed: note "Marketplace listing pending" in Devpost description |
| 22 | Prompt Opinion A2A listing (Path B) live | [ ] | | Or KS-1 executed: note "Marketplace listing pending" in Devpost description |
| 23 | Marketplace listings link back to GitHub repo | [ ] | | Verify bidirectional links: listing → repo AND README → listing |

## Final verification

| # | Item | Check | Signed off by | Notes |
|---|------|:-----:|:-------------:|-------|
| 24 | Demo URL (if deployed) still reachable from fresh browser | [ ] | | Open incognito window, navigate to Vercel URL. If local-only, mark N/A. |
| 25 | Devpost description mentions: HAPI FHIR, MCP, A2A, and links repo + video + demo | [ ] | | All four protocols/tools named in description text |
...
| 14 | No captions | YouTube auto-generate → download SRT → upload alongside video | 15 min |
| 17 | Devpost still Draft | Click Submit. No excuses. KS-6: ship last clean take if video incomplete. | 2 min |
| 21-22 | Marketplace blocked | Execute KS-1: add "Marketplace listing pending" note to Devpost description with link to draft listing | 5 min |
