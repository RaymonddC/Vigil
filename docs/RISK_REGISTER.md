...
|----|------|----------|---------|---------|-------|------------|-------|-------------------|-------------|
| R01 | Prompt Opinion publishing flow is undocumented; we cannot list the MCP server or A2A agent before the deadline. | Platform | 4 | 5 | 20 | Create PO account day 1. Reverse-engineer from `github.com/prompt-opinion/po-adk-python` and any public listing manifest. Ask in Discord early. | Tech Lead | 48h before deadline with no successful listing dry-run. | Publish as GitHub repo + standalone Devpost video. Note "pending marketplace approval" in submission. |
